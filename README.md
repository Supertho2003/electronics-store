...
## Hướng Dẫn Xây Dựng Ứng Dụng

Sau khi kéo mã nguồn về, bạn cần xây dựng tệp `.jar` bằng cách chạy lệnh sau trong thư mục gốc của dự án:
...
1. Sao chép file mẫu `.env.example` thành `.env` trong thư mục gốc của dự án (Electronic-store) và thêm các thông tin cấu hình của bạn vào nhé.

   ```bash
   cp .env.example .env
   ```

2.Tiếp theo đến thư mục ./shop_back_end/src/main/java/dev/com/shop_backend/security/config/VNPAYConfig.java bạn thêm 2 cấu hình
...
Chạy lệnh `docker-compose up --build` trong thư mục gốc của dự án (Electronic-store) để khởi động toàn bộ dự án, bao gồm Frontend, Backend và Database.
